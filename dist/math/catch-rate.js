...
function getCurrentHp(maxHp, currHp) {
    if (currHp === 1) {
        return 1;
    }
...
function getBallBonus(ball, extra) {
    if ((0, utils_1.isUltraBeast)(extra.pokemon) &&
        ball !== 'beast-ball' &&
        ball !== 'master-ball') {
        return 410 / 4096;
    }
    switch (ball) {
        ...
        case 'love-ball':
            if (extra.ownPokemon) {
                if (extra.ownPokemon.name === extra.pokemon &&
            ...
            return 1;
        case 'moon-ball':
            if ((0, utils_1.evolvesFromMoonStone)(extra.pokemon)) {
                return 4;
            }
            else {
                return 1;
        ...
        case 'dream-ball':
            if (extra.status === 'asleep' || (0, utils_1.hasComatoseAbility)(extra.pokemon)) {
                return 4;
            }
            return 1;
        default:
...
function getRateModifier(rate, weight, ball) {
    let modifier = rate;
    if (ball === 'heavy-ball') {
        if (weight >= 3000) {
            modifier += 30;
...
function getBadgePenalty(badges, level) {
...
function getCapturePower(capturePower, offGuard) {
    const capture_powers = [1, 1.1, 1.25, 2];
    return (capture_powers[capturePower] || 1) * (offGuard ? 2 : 1);
}
function pokedexCriticalValue(pokedex) {
 ...
 */
function catchRateGenIX(params) {
    ...
    const maxhp = (0, stat_1.getStatValue)({
        isHp: true,
        iv: (_a = params.iv) !== null && _a !== void 0 ? _a : 31,
        level: params.level,
        base: hpStat.base_stat,
    });
    const hpParam = 3 * maxhp - 2 * getCurrentHp(maxhp, (_b = params.currHp) !== null && _b !== void 0 ? _b : '100%');
    const darkGrass = 1;
    const captureRate = getRateModifier(params.captureRate, pokemon.weight, params.ball);
    ...
    const ballBonus = getBallBonus(params.ball, {
        ...
        turn: (_c = params.turn) !== null && _c !== void 0 ? _c : 1,
        cave: (_d = params.cave) !== null && _d !== void 0 ? _d : false,
        level: params.level,
        night: (_e = params.night) !== null && _e !== void 0 ? _e : false,
        gender: params.gender,
        status: (_f = params.status) !== null && _f !== void 0 ? _f : 'none',
        onWater: (_g = params.onWater) !== null && _g !== void 0 ? _g : false,
        underwater: (_h = params.underwater) !== null && _h !== void 0 ? _h : false,
        registered: (_j = params.registered) !== null && _j !== void 0 ? _j : false,
        ownPokemon: params.ownPokemon,
    });
    const statusMod = getStatus((_k = params.status) !== null && _k !== void 0 ? _k : 'none');
    const badgePenalty = getBadgePenalty((_l = params.badges) !== null && _l !== void 0 ? _l : 8, params.level);
    const lowLevelModifier = params.level < 13 ? (36 - 2 * params.level) / 10 : 1;
    const difficultMod = getCapturePower((_m = params.capturePower) !== null && _m !== void 0 ? _m : 0, (_o = params.offGuard) !== null && _o !== void 0 ? _o : false);
    const r1 = round(round(round(hpParam * darkGrass) * captureRate * ballBonus) * badgePenalty);
    const d1 = down((r1 / (3 * maxhp)) * lowLevelModifier);
    const x = round(d1 * statusMod * difficultMod);
    const cvd = round(65536 / round(Math.fround(Math.pow(round(255 / x), 3 / 16))));
    ...
    const p = pokedexCriticalValue((_p = params.pokedex) !== null && _p !== void 0 ? _p : 0);
    const cc = params.raid
        ? 0
        : Math.floor(round(Math.min(x, 255) * p * charm) / 6);
    const critChance = Math.min(1, cc / 256);
    const finalchance = critChance * breakoutAttempt + (1 - critChance) * successChance;
    return {
